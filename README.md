...
3. To enhance the existing Starbucks heatmap and identify neighborhoods that may be receptive to the new take-out-only store model, US census data on population density, growth, can be overlaid onto it. Furthermore, incorporating Yelp data can assist in identifying areas with existing business competition, specifically shops that serve coffee & tea, which can help determine whether a new Starbucks store should be built in that area.
